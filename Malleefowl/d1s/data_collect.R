...
ConfI <- data.frame(matrix(ncol = 3, nrow = 230), row.names = counts[,"t"])
...
Pis <- data.frame(matrix(ncol = 100, nrow = 47), row.names = sort(append(as.character(1:46*5-5), "229")))
for (j in 1:100){
  for (i in 1:47){
    Pis[i,j] <- mean(2*pis[,i*4+(j)*188]*(1-pis[,i*4+(j)*188]))
  }
}
ConfIP <- data.frame(matrix(ncol = 3, nrow = 47), row.names = sort(append(as.character(1:46*5-5), "229")))
...
######If use SD#####################################################
#for (i in 1:47){
#  ConfIP[i,1] <- mean(unlist((c(pis[,(1:100)*188+i*4]))))
#  ConfIP[i,2] <- ConfIP[i,1]-sd(unlist((c(pis[,(1:100)*188+i*4]))))
#  ConfIP[i,3] <- ConfIP[i,1]+sd(unlist((c(pis[,(1:100)*188+i*4]))))
#}
####################################################################
...
Fixed <- data.frame(matrix(ncol = 100, nrow = 47), row.names = sort(append(as.character(1:46*5-5), "229")))

for (j in 1:100){
  for (i in 1:47){
  Fixed[i,j] <- sum(pis[,i*4+(j)*188]==0)
  }
}  

ConfIF <- data.frame(matrix(ncol = 3, nrow = 47), row.names = sort(append(as.character(1:46*5-5), "229")))
